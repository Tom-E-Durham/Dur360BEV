from src.models.layers.common import SparseUpsamplingAdd, SubMConvNormReLU
...
class SparseBasicBlock(spconv.SparseModule):
    ...
    def __init__(
        ...
        conv1 = layer(in_c, out_c, kernels[0],
                      strides[0], 1, bias=False, algo=algo)
        conv2 = layer(out_c, out_c, kernels[1],
                      strides[1], 1, bias=False, algo=algo)
...
class SparseEncoder(nn.Module):
    def __init__(
        ...
        self.first_conv = SubMConvNormReLU(
            in_c, mid_c, kernel_size=7, activ=activ)
        ...
        self.layer_2 = spconv.SparseSequential(
            SparseBasicBlock(
                ...
                downsample=spconv.SparseSequential(
                    spconv.SubMConv2d(mid_c, mid_c_lev2, 1,
        ...
        self.layer_3 = spconv.SparseSequential(
            SparseBasicBlock(
                ...
                kernels=[3, 3],
                downsample=spconv.SparseSequential(
                    spconv.SubMConv2d(mid_c_lev2, mid_c_lev3,
                                      1, bias=False, algo=algo),
        ...
        if down_mode == "maxpool":
            ...
            self.down_3 = spconv.SparseMaxPool2d(
                3, 2, 1, indice_key="cp3", algo=algo)
        else:
...
class SparseUNet(nn.Module):
    def __init__(
        ...
        self.encoder = SparseEncoder(
            in_c, mid_c, "maxpool", with_large_kernels)
        self.decoder = SparseDecoder(
            mid_c, in_c, with_large_kernels, with_decoder_bias)
        self.in_c = in_c

        if with_tail_conv:
            self.tail_conv = spconv.SparseSequential(
                SubMConvNormReLU(in_c, out_c, kernel_size=7, algo=algo),
                spconv.SubMConv2d(out_c, out_c, kernel_size=1,
                                  bias=False, algo=algo),
            )
        else:
        ...
        **kwargs,
        ...
        if not from_dense:
            ...
            x_sp = spconv.SparseConvTensor(
                feats, indices, spatial_shape, batch_size)
        else:
