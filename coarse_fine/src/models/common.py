...
from src.models.projector import CamProjector
...
class Network(nn.Module):
    ...
    def _360_init_dict_shape(self, imgs):
        """[Dur360 revised]
    ...
    # Common begining
    def _360_common_init_backneck_prepare_vt(self, imgs):
        """[Dur360 revised]
        This has been changed to get feature tensor like: [b, c, h, w]
        """
        # Dict shape and vox.
        dict_shape = self._360_init_dict_shape(imgs)
        dict_vox = self._init_dict_vox()

        # Extract image features.
        img_feats = self.forward_backneck_360(imgs)
        dict_shape["Hfeats"] = img_feats.size(-2)
        dict_shape["Wfeats"] = img_feats.size(-1)
        dict_img = {"img_feats": img_feats}

        return dict_shape, dict_vox, dict_img, None
    ...
    def forward_backneck_360(self, imgs):
        """[Dur360 revised]
        Backbone and Neck
        removed arranging shape process before and after backbone
        """
        imgs_feats = self.neck(self.backbone(imgs))
        return imgs_feats
    # VT
    ...
    def _prepare_view_transform(self, rots, trans, intrins):
        ...
        trans = rearrange(trans, "b t n i 1 -> (b t) n i 1",
                          b=b, t=t, n=n, i=3)
    ...
    def _fuse_b_nq(self, bev_feats):
        ...
        bev_feats = rearrange(
            bev_feats, "b nq c h w -> (b nq) c h w", b=b, nq=nq)
        return bev_feats
    ...
    def forward(self, imgs, rots, trans, intrins, bev_aug, egoTin_to_seq, **kwargs):
        (
            ...
            dict_img,
            dict_mat,
        ) = self._360_common_init_backneck_prepare_vt(
