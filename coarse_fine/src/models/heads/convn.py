...
from src.models.layers.common import ConvNormAct, SubMConvNormAct
...
class BEVConvHead(nn.Module):
    ...
    def _get_layers(self, dict_input, shared_out_c, dense_input):
        ...
        if dense_input:
            ...
            conv_c1 = nn.Conv2d(shared_out_c, out_channels=1,
                                kernel_size=1, padding=0)
            convnormact_conv_c1 = nn.Sequential(
                ConvNormAct(shared_out_c, shared_out_c,
                            3, 1, False, norm, activ),
                deepcopy(conv_c1),
            )
            convnormact_conv_c2 = nn.Sequential(
                ConvNormAct(shared_out_c, shared_out_c,
                            3, 1, False, norm, activ),
                nn.Conv2d(shared_out_c, out_channels=2,
                          kernel_size=1, padding=0),
            )
            if with_hdmap:
            ...
            convnormact_conv_c1 = spconv.SparseSequential(
                SubMConvNormAct(shared_out_c, shared_out_c,
                                3, 1, False, norm, activ),
                deepcopy(conv_c1),
            )
    ...
    def _apply_final_activation(self, out_dict):
        ...
        if self.dense_input:
            def apply_activ_(x, func): return func(x)
        else:
            def apply_activ_(x, func): return x.replace_feature(
                func(x.features))

        if self.with_centr_offs:
            feats = out_dict["centerness"]
            out_dict["centerness"] = apply_activ_(
                feats, lambda x: F.sigmoid(x))
        return out_dict
