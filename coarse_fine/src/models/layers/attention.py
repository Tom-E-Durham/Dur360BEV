...
try:
    from src.ops.defattn.modules import MSDeformAttn, MSDeformAttn3D
except:
...
class SADefnAttn(nn.Module):
    ...
    def forward(self, query, query_pos=None):
        ...
        ref_points = repeat(
            ref_points, "zx coords -> b zx coords", b=B, coords=2)
...
class CADefnAttn(nn.Module):
    ...
    def forward(
        ...
        max_len = max([max([len(idx) for idx in idx_b])
                       for idx_b in idx_per_cam])
        ...
        for i, idx_query_cam in enumerate(idx_per_cam):
            for j in range(b):
                query_out[j, idx_query_cam[j]] += queries[j,
                                                          i, : len(idx_query_cam[j])]
        ...
        if edges is None:
            if self.register:
                x_o, attn_weights = self.attnlayer(
                    x_o, return_attention=self.register)
