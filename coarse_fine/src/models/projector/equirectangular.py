"""
Original Author: Loick Chambon

Modified by: Wenke E
Email: wenke.e@durham.ac.uk

Description:
- Modified to project 3D points to 2D equirectaunglar images.
...
class EquiCamProjector(nn.Module):
    def __init__(
        self,
        spatial_bounds: List[float] = [-49.75,
                                       49.75, -49.75, 49.75, -3.375, 5.375],
        voxel_ref="spatial",
    ...
    def from_voxel_ref_to_cams(self, vox_coords, rots, trans, bev_aug, egoTin_to_seq):
        ...
        """
        vox_coords = self.from_spatial_to_seqaug(
            vox_coords, bev_aug, egoTin_to_seq)
        voxcam_coords = self.from_spatial_to_cams(vox_coords, rots, trans)

        return voxcam_coords, vox_coords

    # Voxel to cams
    def _360_from_voxel_ref_to_cams(self, vox_coords):
        """Project points from voxel reference to camera reference.
        Args:
            - rots, trans: map points from cameras to ego. In Nuscenes, extrinsics
            are inverted compared to standard conventions. They map sensors to ego.
        ...
        # get voxcam_coords
        b = vox_coords.shape[0]
        vox_coords_ = rearrange(vox_coords, "b i x y z -> b (x y z) i", i=3)
        _vox_coords_ = vox_coords_[:, :, [1, 2, 0]]

        # Rotate coordinates
        _vox_coords_ = rotate_y_axis(
            _vox_coords_, angle=torch.tensor(-np.pi / 2))
        ...
        voxcam_coords = torch.stack([x, y, z], axis=2)

        voxcam_coords = rearrange(
            voxcam_coords,
            "b (zcam xcam ycam) i -> b 1 zcam ycam xcam i",
            b=b,
            i=3,
            zcam=self.Z_cam,
            xcam=self.X_cam,
        ...
        # arrange the shape of vox_coords
        vox_coords = rearrange(vox_coords, "b i x y z -> b 1 x z y i", i=3)
    ...
    def from_spatial_to_seqaug(self, vox_coords, bev_aug, egoTin_to_seq):
        ...
        vox_coords = torch.cat(
            [vox_coords, torch.ones_like(vox_coords[:, :1])], dim=1)
        vox_coords_aug = torch.bmm(
            bev_aug, torch.bmm(egoTin_to_seq, vox_coords))
    ...
    def from_cameras_to_pixels(self, voxels, intrins):
        ...
        intrins = rearrange(
            intrins, "bt n i j -> (bt n) i j", bt=bt, n=n, i=i, j=j)
        voxels = rearrange(
            voxels, "bt n i Npts -> (bt n) i Npts", bt=bt, n=n, i=3)
    ...
    def modify_z_value(self, voxels, z_before_norm, z_value_mode: bool = True):
        ...
        if z_value_mode != "zero":
            ...
            z_before_norm = z_before_norm / \
                (max(abs(zmin), abs(zmax)) * 1.4142135)
    ...
    # Forward
    def _forward(self, dict_mat, dict_shape, dict_vox) -> Dict[str, torch.Tensor]:
        # Unpack
        ...
        if self.z_value_mode != "zero":
            # Get z before normalization.
            z_before_norm = voxcam_coords[:, :, 2:3]
        else:
            z_before_norm = None
        voxcam_coords = self.normalize_z_cam(voxcam_coords)
        x_valid, y_valid = self.valid_points_in_pixels(
            voxcam_coords, img_feats_res)
    ...
    def forward(self, dict_mat, dict_shape, dict_vox) -> Dict[str, torch.Tensor]:
        ...
        # Ego to cams.
        voxcam_coords, vox_coords = self._360_from_voxel_ref_to_cams(
            vox_coords,
        )

        vox_valid = torch.ones_like(voxcam_coords)[:, :, :, :, :, :1]
