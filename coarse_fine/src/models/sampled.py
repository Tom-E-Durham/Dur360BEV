"""
Modified version of the PointBeV project (https://github.com/valeoai/PointBeV).

Modifications by: Wenke E (wenke.e@durham.ac.uk)
"""
...
from src.models.common import CoordSelector, Network
from src.utils import list_dict_to_dict_list

from omegaconf import DictConfig
from typing import Union


class Coarse_fine(Network):
    def __init__(
        self,
        # Modules
        backbone=None,
        ...
        self.coord_selector = SampledCoordSelector(
            in_shape, voxel_ref, sampled_kwargs)
        return

    def print_tree(self, cfg: Union[DictConfig, dict], indent_level=0):
        indent = "|"+"--" * indent_level  # Create indentation based on the current level
            ...
            for key in cfg.keys():
                try:
                    ...
                    if isinstance(value, DictConfig):
                        ...
                        self.print_tree(value, indent_level + 1)
                    else:
                        try:
                    ...
                    print(f"{indent}{key}: <Unable to resolve: {e}>")
    ...
    def forward_sampled(
        ...
        # Unpack
        # [b, 6, 128, 28, 60]: [b, cams, channel, hf, wf]
    ...
    def forward_coarse(self, dict_img, dict_mat, dict_vox, dict_shape):
        return self._360_forward_sampled(
        ...
        for d in [dict_out, dict_masks]:
            for k in d.keys():
                for i, elem in enumerate(d[k]):
                    ...
                    if len(elem.shape) == 4:
                        d[k][i] = rearrange(
                            elem, "(b nq) c h w -> b nq c h w", b=b)
                    else:
                        d[k][i] = elem
                    # Keep only present.
                    d[k][i] = d[k][i][:, -1:]
    ...
    def forward_coarse_and_fine(
        ...
        # get vox_coords and vox_idx
        device = dict_img['img_feats'].device
        dict_vox.update(
            self.coord_selector._360_get_vox_coords_and_idx(
                dict_shape, device, sampling_imgs
            )
        )
        ...
        if not bool_skip:
            out, masks = self._process_listdict(
                list_out, list_masks, dict_shape)
        else:
            out = out_coarse
            masks = masks_coarse
    ...
    def forward_temporal(self, bev_feats, dict_shape):
        if not self.temporal:
            ...
            if not self.training:
                ...
                present_indices_xy = (
                    torch.arange(
                        0, X * Y, device=device).repeat(b, 1).flatten(0, 1)
                )
    ...
    # original Forward
    def ori_forward(self, imgs, rots, trans, intrins, bev_aug, egoTin_to_seq, **kwargs):
        (
            dict_shape,
            dict_vox,
            dict_img,
    ...
    def forward(self, imgs):
        (
            ...
            dict_img,
            dict_mat,
        ) = self._360_common_init_backneck_prepare_vt(imgs)
...
class SampledCoordSelector(CoordSelector):
    ...
    def _get_vox_coords_and_idx(
        ...
        # During validation
            ...
            if self.val_mode == "dense":
                ...
                dict_out = self._get_img_pts(
                    sampling_imgs["hdmap"][:, :, :1], device)
            else:
    ...
    def _360_get_vox_coords_and_idx(
        self, dict_shape, device, sampling_imgs={}
    ) -> Dict[str, Tensor]:
        # Alias
        b = dict_shape['b']
        ...
        if self.training:
            if self.mode == "dense":
                dict_out = self._get_dense_coords(b)
            else:
                dict_out = self._get_sparse_coords(
                    b,
                    device,
        ...
        # During validation
            ...
            if self.val_mode == "dense":
                ...
                dict_out = self._get_sparse_coords(b, device, self.val_mode)
        dict_vox.update(dict_out)
    ...
    def _get_img_pts(self, img, device, subsample=True):
        ...
        dist = torch.tensor(
            [abs(sb[0]), abs(sb[2]), abs(sb[4])], device=device)
    ...
    def _get_sparse_coords(
        ...
        pillar_heights = repeat(
            pillar_heights, "z -> bt 1 (xy z)", bt=bt, xy=Nxy)
    ...
    def _get_fine_mask(
        ...
        # Keep top N_anchor points.
            ...
            batch_idx = torch.arange(
                bt, device=device).repeat_interleave(N_anchor)
            ...
            xy_vox_idx = rearrange(
                xy_vox_idx, "(bt N) c -> bt N c", c=2, bt=bt)
