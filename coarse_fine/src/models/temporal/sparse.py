...
from src.models.layers.attention import PositionalEncodingMap
...
class SparseTemporal(nn.Module):
    ...
    def _apply_temp_embd(self, features, indices_t, t):
        if self.embed_mode_apply == "slice":
            ...
            features = features + \
                self.temp_embed((indices_t.view(-1, 1) / t) * 2 - 1)
        else:
    ...
    def forward_sparse_torchgeom(self, bev_feats, b_t):
        ...
        indices = bev_feats.indices
        indices_flat = indices[:, 0] * X * Y + \
            indices[:, 1] * Y + indices[:, 2]

        # Get query, keys: batch x time level.
        img_mask = torch.zeros(
            b * t * X * Y, device=device, dtype=indices.dtype)
        img_mask[indices_flat] = 1
        img_mask = rearrange(
            img_mask, "(b t x y) -> b t x y", b=b, t=t, x=X, y=Y)
