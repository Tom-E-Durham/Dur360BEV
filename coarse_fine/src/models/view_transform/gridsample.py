...
from src.ops.gs.functions import sparsed_grid_sample
from src.utils.debug import debug_hook
...
class GridSampleVT(nn.Module):
    ...
    def get_feats_from_grid_sample(self, voxcam_coords, img_feats, vox_valid):
        ...
        """
        # Alias
        bt, *Npts, coord = voxcam_coords.shape  # (bt ,n, Zcam, Ycam, Xcam, 3)
        bt, c, h, w = img_feats.shape

        vox_valid = rearrange(
            vox_valid,
            "bt zcam ycam xcam 1 -> bt 1 zcam ycam xcam",
            bt=bt,
        )
        img_feats = rearrange(
            img_feats,
            "bt (c ng) h w -> bt c ng h w",
            bt=bt,
            c=c,
            h=h,
            w=w,
    ...
    def get_feats_sparse_grid_sample(self, voxcam_coords, img_feats, vox_valid, shapes):
        ...
        vox_valid_flat = rearrange(
            vox_valid, "bt 1 zcam ycam xcam -> (bt zcam ycam xcam)", bt=bt
        )

        index = torch.nonzero(vox_valid_flat).squeeze(1)
        index_cam = (index // prod(Npts)) % 1
        index_batch = (index // (prod(Npts) * 1)) % bt
        ...
        sparse_vox_feats = sparsed_grid_sample(
            img_feats, select_vox, (index_batch * 1 +
                                    index_cam).to(torch.int16)
        )
    ...
    def reduce_cams_sparse(self, sparse_vox_feats, index, shapes):
        ...
        index_batch = (index // (prod(Npts))) % bt
        ...
        out = torch.zeros((Nactiv_pts, c), device=device,
                          dtype=sparse_vox_feats.dtype)
        sparse_vox_feats_sum = out.index_add_(
            dim=0, index=inv_idx, source=sparse_vox_feats
        )

        # mean
        cnt = torch.zeros((Nactiv_pts, c), device=device,
                          dtype=sparse_vox_feats.dtype)
        cnt = cnt.index_add_(
    ...
    def forward_cam(
        ...
        if not self.grid_sample_mode == "sparse_optim":
            ...
            bt, *Npts, _ = voxcam_coords.shape
            vox_feats = self.reduce_cams_sparse(vox_feats, index, (bt, Npts))
        return vox_feats
    ...
    def _arrange_dense_outputs(self, vox_feats, vox_idx, b_t, Z_bins, X_bins):
        ...
        mask = rearrange(
            mask, "(b t) 1 zcam xcam -> b t 1 zcam xcam", b=b, t=t)
    ...
    def _arrange_sparse_outputs(
        ...
        vox_feats = rearrange(
            vox_feats, "b t c zcam xcam -> (b t zcam xcam) c")
        ...
        if self.return_sparse:
            ...
            indices = torch.cat(
                [vox_idx[..., -1:], vox_idx[..., :2]], dim=-1).int()
            # Flip
            indices[..., 1:] = (
                torch.tensor([Z_bins - 1, X_bins - 1],
                             device=device) - indices[..., 1:]
            )
            # Get mask from indices.
            mask_ = spconv.SparseConvTensor(
                torch.ones_like(indices[..., :1]), indices, [
                    Z_bins, X_bins], bt
            )
        ...
        out = torch.scatter(out, 0, indices.expand(
            indices.shape[0], c), vox_feats)
        out = rearrange(out, "(h w b) c -> b c h w", b=bt, h=Z_bins, w=X_bins)
        mask = torch.scatter(
            mask, 0, indices, torch.ones_like(indices, dtype=torch.float32)
        ).bool()
        mask = rearrange(mask, "(h w b) c -> b c h w",
                         b=bt, h=Z_bins, w=X_bins)
    ...
    def forward(self, img_feats, dict_vox, model_3D_processor=None):
        ...
        # Alias
        (b, t, *_) = voxcam_coords.shape

        self._set_axis(voxcam_coords)

        # Prepare
        voxcam_coords = rearrange(
            voxcam_coords, "b t zcam ycam xcam i -> (b t) zcam ycam xcam i", i=3
        )
        vox_valid = rearrange(
            vox_valid, "b t zcam ycam xcam i -> (b t) zcam ycam xcam i", i=1
        )
        ...
        vox_feats = self.forward_cam(
            voxcam_coords, H_W, vox_feats, vox_valid, index)
