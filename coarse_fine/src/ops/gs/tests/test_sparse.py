...
def test_compare_sparse(input_data, is_2d, w_torch, w_pckg):
    ...
    if w_torch:
        if is_2d:
            assert not W_MODULE
        out_torch_fw, out_torch_bw = gs_torch(
            is_2d, inp_torch, grid_torch, grid_mask_torch)
    if w_pckg:
        out_sparse_fw, out_sparse_bw = gs_sparse_pckg(
            inp_pckg, grid_pckg_, index_batch)
    print('\n================================')
    # fmt: on
...
def test_dtype(dtype_inp_grid, dtype_index):
    ...
    index_batch = torch.randint(
        0, N, (Npts,), device="cuda", dtype=dtype_index)
