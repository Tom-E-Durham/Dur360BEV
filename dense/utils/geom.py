...
def eye_4x4(B, device='cuda'):
    rt = torch.eye(4, device=torch.device(device)).view(
        1, 4, 4).repeat([B, 1, 1])
    return rt
...
def get_camM_T_camXs(origin_T_camXs, ind=0):
    ...
    for b in list(range(B)):
        ...
        for s in list(range(S)):
            camM_T_camXs[b, s] = torch.matmul(
                camM_T_origin, origin_T_camXs[b, s])
    return camM_T_camXs
...
def merge_rtlist(rlist, tlist):
    ...
    def __p(x): return utils.basic.pack_seqdim(x, B)
    def __u(x): return utils.basic.unpack_seqdim(x, B)
...
def get_xyzlist_from_lenlist(lenlist):
    ...
    xs = torch.stack([lx/2., lx/2., -lx/2., -lx/2., lx /
                      2., lx/2., -lx/2., -lx/2.], axis=2)
    ys = torch.stack([ly/2., ly/2., ly/2., ly/2., -ly /
                      2., -ly/2., -ly/2., -ly/2.], axis=2)
    zs = torch.stack([lz/2., -lz/2., -lz/2., lz/2., lz /
                      2., -lz/2., -lz/2., lz/2.], axis=2)
