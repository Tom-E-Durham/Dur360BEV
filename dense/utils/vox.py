...
class Vox_util(object):
    ...
    def Ref2Mem(self, xyz, Z, Y, X, assert_cube=False):
        ...
        device = xyz.device
        assert(C == 3)
        mem_T_ref = self.get_mem_T_ref(
            B, Z, Y, X, assert_cube=assert_cube, device=device)
    ...
    def Mem2Ref(self, xyz_mem, Z, Y, X, assert_cube=False):
        ...
        ref_T_mem = self.get_ref_T_mem(
            B, Z, Y, X, assert_cube=assert_cube, device=xyz_mem.device)
    ...
    def get_inbounds(self, xyz, Z, Y, X, already_mem=False, padding=0.0, assert_cube=False):
        ...
        x_valid = ((x-padding) > -0.5).byte() & ((x+padding)
                                                 < float(X-0.5)).byte()
        y_valid = ((y-padding) > -0.5).byte() & ((y+padding)
                                                 < float(Y-0.5)).byte()
        z_valid = ((z-padding) > -0.5).byte() & ((z+padding)
                                                 < float(Z-0.5)).byte()
        nonzero = (~(z == 0.0)).byte()
    ...
    def voxelize_xyz(self, xyz_ref, Z, Y, X, already_mem=False, assert_cube=False, clean_eps=0):
        ...
        if already_mem:
            ...
            xyz_zero = self.Ref2Mem(
                xyz_ref[:, 0:1]*0, Z, Y, X, assert_cube=assert_cube)
        vox = self.get_occupancy(
            xyz_mem, Z, Y, X, clean_eps=clean_eps, xyz_zero=xyz_zero)
        return vox
    ...
    def get_feat_occupancy(self, xyz, feat, Z, Y, X, clean_eps=0, xyz_zero=None):
        ...
        feat_voxels = feat_voxels.reshape(
            B, Z, Y, X, D2).permute(0, 4, 1, 2, 3)
        # B x C x Z x Y x X
        return feat_voxels
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False, xyz_camA=None):
        ...
        if xyz_camA is None:
            xyz_memA = utils.basic.gridcloud3d(
                B, Z, Y, X, norm=False, device=pixB_T_camA.device)
        ...
        if (0):
            ...
            for b in list(range(B)):
                values[b] = utils.samp.bilinear_sample_single(
    ...
    def warp_tiled_to_mem(self, rgb_tileB, pixB_T_camA, camB_T_camA, Z, Y, X, DMIN, DMAX, assert_cube=False):
        ...
        z_tileB, y_pixB, x_pixB = utils.basic.normalize_grid3d(
            z_tileB, y, x, D, H, W)
    ...
    def apply_mem_T_ref_to_lrtlist(self, lrtlist_cam, Z, Y, X, assert_cube=False):
        ...
        assert(C == 19)
        mem_T_cam = self.get_mem_T_ref(
            B, Z, Y, X, assert_cube=assert_cube, device=lrtlist_cam.device)
    ...
    def xyz2circles(self, xyz, radius, Z, Y, X, soft=True, already_mem=True, also_offset=False, grid=None):
        ...
        if grid is None:
            grid_z, grid_y, grid_x = utils.basic.meshgrid3d(
                B, Z, Y, X, stack=False, norm=False, device=xyz.device)
        ...
        if soft:
            ...
            # something is wrong with this. come back later to debug
            assert(False)
            ...
            within_radius_mask = torch.sum(
                within_radius_mask, dim=1, keepdim=True).clamp(0, 1)
    ...
    def xyz2circles_bev(self, xyz, radius, Z, Y, X, already_mem=True, also_offset=False):
        ...
        grid_z, grid_x = utils.basic.meshgrid2d(
            B, Z, X, stack=False, norm=False, device=xyz.device)
        # note the default stack is on -1
